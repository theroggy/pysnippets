...
if __name__ == "__main__":
    ...
    if not buildings_path.exists():
        for path in buildings_paths:
            gfo.copy_layer(src=path, dst=buildings_path, dst_layer=buildings_path.stem, append=True, columns=building_cols)

    parcels_path = alkis_dir / "NutzungFlurstueck.gpkg"
    if not parcels_path.exists():
        for path in parcels_paths:
            gfo.copy_layer(src=path, dst=parcels_path, dst_layer=parcels_path.stem, append=True, columns=parcels_cols)
    ...
    gfo.intersection(buildings_path, parcels_path, buildings_with_parcels_path)
