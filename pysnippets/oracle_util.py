...
def read_oracle(
    connectionstring: str, sql_stmt: str, dst_crs: Optional[pyproj.CRS] = None
) -> gpd.GeoDataFrame:
    """
    ...
    try:
        ...
        if "GEOMETRY_WKB" not in columns:
            raise ValueError(
                "sql_stmt must select a column called GEOMETRY_WKB! Use "
                "SDO_UTIL.TO_WKBGEOMETRY() to convert a SDO_GEOMETRY to wkb."
            )
